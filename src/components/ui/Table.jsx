          ...
          <tbody className="divide-y divide-gray-200">
            {data.length === 0 ? (
              ...
              data.map((row, rowIndex) => (
                <tr key={row.id || rowIndex} className={rowIndex % 2 === 0 ? "bg-gray-50" : "bg-gray-100"}>
                  {columns.map((col) => (
      ...
      <div className="md:hidden space-y-4">
        {data.length === 0 ? (
          ...
          data.map((row, rowIndex) => (
            <div
              key={row.id || rowIndex}
              className={`rounded shadow divide-y ${rowIndex % 2 === 0 ? "bg-gray-50" : "bg-gray-100"} divide-gray-300`}
            >
              {columns.map((col) => (
